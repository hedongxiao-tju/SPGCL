...
def main(args):
    ...
    for i in range(args.runs):
        ...
        with torch.no_grad():
            ...
            z = out[0]
        m, n = evaluate_clustering(z, data)
        print(m, n)
        run_means.append(m)
        with open(log_dir, "a") as f:
            f.write("NMI: " + str(m)[0:7] + "H-score: " + str(n))
            f.write("\n")
            
    if len(run_means) > 1:
        avg_score = float(np.mean(run_means))
        print(f"Average NMI over {args.runs} runs: {avg_score}")
        with open(log_dir, "a") as f:
            f.write(f"Average NMI over {args.runs} runs: {avg_score}\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser("SPGCL")
    ...
    parser.add_argument("--log_dir", type=str, default="./log/cluster/Photo", help="Path to log")
    ...
    parser.add_argument("--neighbor_masking_prob", type=float, default=0.2, help="Edge dropout probability")
    ...
    parser.add_argument("--edepoch", type=int, default=0, help="epoch to start edge dropping")
